...
// One node of a parsed vanilla help syntax tree
...
// One base command with its argument tree
...
type VanillaEngine struct {
	...
	argumentMappings map[string]func() []MappedValue
	expandedPaths    map[string]bool
}

// Builds an engine that reads help over the command provider
func NewVanillaEngine(commandProvider CommandProvider, playerListProvider PlayerListProvider) *VanillaEngine {
	return &VanillaEngine{
		helpFunc:         helpLookup(commandProvider),
		argumentMappings: vanillaArgumentMappings(playerListProvider),
		expandedPaths:    make(map[string]bool),
	}
}

// Maps argument names to their known values
func vanillaArgumentMappings(playerListProvider PlayerListProvider) map[string]func() []MappedValue {
	return map[string]func() []MappedValue{
		"gamemode": func() []MappedValue {
			return []MappedValue{
				{Text: "adventure"}, {Text: "survival"}, {Text: "creative"}, {Text: "spectator"},
			}
		},
		"targets": func() []MappedValue {
			var targets []MappedValue

			if playerListProvider != nil {
				players, err := playerListProvider.GetPlayers()
				if err == nil && len(players) > 0 {
					for _, p := range players {
						targets = append(targets, MappedValue{Text: p, IsPlayer: true})
					}
				}
			}
			for _, sel := range []string{"@a", "@e", "@n", "@s", "@p", "@r"} {
				targets = append(targets, MappedValue{Text: sel, IsPlayer: false})
			}

			return targets
		},
		"target": func() []MappedValue {
			var targets []MappedValue
...
func (e *VanillaEngine) GetPredictions(command string) ([]*Token, error) {
	...
	for i, token := range remainingTokens {
		...
		// Detects a wildcard node at this level
		hasWildcard := false
					...
					if mappingFunc, exists := e.argumentMappings[node.Text]; exists {
						for _, mv := range mappingFunc() {
							if strings.HasPrefix(mv.Text, token) {
								...
								predictions = append(predictions, &Token{
									Text:       mv.Text,
									IsArgument: true,
...
func cleanHelpOutput(rawHelpString string) string {
	...
	return strings.TrimSpace(StripMinecraftColors(joined))
}
...
func (e *VanillaEngine) loadCommandsFromRawHelpWithQuery(rawHelpString string, queryPath []string) []*VanillaCommand {
	...
	for _, rawCommand := range rawCommands {
		...
		// Strips query path prefix and repeated schema parameters
		if len(queryPath) > 0 && len(rawTokens) >= len(queryPath) {
			...
			if matchesQuery {
				...
				// Drops leading tokens that repeat query path arguments
				for len(remaining) > 0 {
				...
				// Treats a repeated last query token as fallback help
				if len(remaining) > 0 {
				...
				// Rebuilds raw tokens from query path and remainder
...
// Splits one raw help token into its alternatives
