...
Every start, the panel writes a recovery key to `recovery.key` in its data directory (and prints it to the log). The **Recovery** link on the login page accepts it - and resets the panel to first-user setup, wiping all accounts, sessions, tokens and invites. Running modules are rebuilt with fresh tokens. Servers and their data are untouched. It's the fire axe, not the spare key.
