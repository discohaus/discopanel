// Settles older era schema leftovers onto a migration target
package migrations
...
// Auto migrate never dropped anything, the backup holds it
func conformToTarget(target *migrate.Spec) migrate.Transform {
	return migrate.Transform{Name: "conform_target", Fn: func(tx *gorm.DB, d migrate.Dialect) error {
		observed, err := migrate.SpecOfDB(tx)
		if err != nil {
			return err
		}
		carryIndexWhere(observed, target)
		ops, demands, err := migrate.Diff(observed, target, confirmAll(observed, target))
		if err != nil {
			return err
		}
		if len(demands) > 0 {
...
func confirmAll(observed, target *migrate.Spec) *migrate.Resolution {
